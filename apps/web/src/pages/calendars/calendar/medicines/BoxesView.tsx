...
function BoxesView({ personalCalendars, sharedUserCalendars, tokenCalendars }: BoxesViewPageProps) {
  ...
  return (
    <div className="container mx-auto flex flex-col items-center gap-4">
      ...
      {/* Edit Box Dialog */}
      <Dialog open={editingBoxId !== null} onOpenChange={(open) => { if (!open) cancelEditing(); }}>
        <DialogContent className="max-w-lg grid-rows-[auto_minmax(0,1fr)]! max-h-[85vh] overflow-hidden">
          <DialogHeader>
            <DialogTitle>
              {String(editingBoxId || '').startsWith('temp-') ? t('boxes.add_manual') : t('boxes.edit')}
            </DialogTitle>
